    ...
    doc.forEach(function (doc){
      skærme.push(doc.id);
          ...
          doc.forEach(function (doc){
            ...
            .get()
            .then((documentQuery) => {documentQuery.forEach((doc)=>{
              vistPå[slideshow].push(doc.id);  
            })});
            slideshows.push(doc.id);
          })

      res.render(__dirname+'/public/views/forside', {skærme, slideshows, link:`./rediger/sk%C3%A6rme/`})
          });
