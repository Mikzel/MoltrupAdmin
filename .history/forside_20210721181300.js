  ...
  db.collection(`slideshows`).doc(slideshowId).get().then((snapshotQuery) => {
    if (snapshotQuery.exists){
      console.log(snapshotQuery.data());
              ...
              db.collection(`slideshows`).doc(slideshowId).collection(`slideshows`).get().then((snapshotQuery) => {
               
                
              })

      res.render(__dirname+'/public/views/MasterSlide.ejs', {slideshow: snapshotQuery.data()})
    }else{
