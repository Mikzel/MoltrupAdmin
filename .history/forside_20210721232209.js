          ...
          if (snapshotQuery.exists){
            ...
            let slideshowNavn = slideshowId.replace(/_/g, ' ');
                db.collection(`slideshows`).doc(slideshowId).collection('slides').get().then((snapshotQuery) => {
                  ...
                  async()=>{
                   await snapshotQuery.forEach((doc)=>{if(doc.vis){slides.push({id:doc.id, data:doc.data()})}});
                    let sortSlides = await slides.sort(function(a, b) {return a.data.pos - b.data.pos;});
