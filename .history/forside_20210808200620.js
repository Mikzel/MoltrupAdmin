        ...
        doc.forEach(function (doc){
          skærme.push(doc.id);
              ...
              if (st == antal) {
               setTimeout(() =>{ 
                 console.log(vistPå);
                res.render(__dirname+'/public/views/forside', {skærme, slideshows, vistPå, link:`./rediger/sk%C3%A6rme/`, brugernavn: loginData.name});
             },1000)
  ...
  let skabsnap = docSnapshot.data();
      ...
      try {
        ...
        if (!query.size) {
          widgets ='';
          brugte=[];
          res.render(__dirname+'/public/views/skaerm', {brugernavn: loginData.name,skærm:skaerm, skabelon:skabsnap, widgets: widgetList, brugte:brugte});
        }else{
    ...
    db.collection(`slideshows`).doc(slideshowId).get().then((snapshotQuery) => {
      if (snapshotQuery.exists){
        ...
        let slideshowNavn = slideshowId.replace(/_/g, ' ');
                ...
                res.render(__dirname+'/public/views/MasterSlide.ejs', {brugernavn: loginData.name,slides, slideshow, slideshowId, slideshowNavn});
