...
app.get('/rediger/slideshows/:slideshowId', (req, res) => {
  ...
  db.doc(`slideshows`).collection(slideshowId).get().then((docSnapshot) => {
    if (docSnapshot.exists){
      console.log(docSnapshot.data());


      let slides = [];
        db.doc(`slideshows/${slideshowId}/slides`).get().then((snapShot) => {
          if (snapShot.exists){snapShot.docs.forEach((doc)=>{})}
        })

      res.render(__dirname+'/public/views/MasterSlide.ejs', {slideshow: docSnapshot.data()})
    }else{
