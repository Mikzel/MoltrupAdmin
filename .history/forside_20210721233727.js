          ...
          if (snapshotQuery.exists){
            ...
            var process = 0;
                db.collection(`slideshows`).doc(slideshowId).collection('slides').get().then((snapshotQuery) => {
                   ...
                   snapshotQuery.forEach((doc)=>{if(doc.vis){slides.push({id:doc.id, data:doc.data()})} 
                   process++
                   if(process => snapshotQuery.length){
                     console.log(process);
                    let sortSlides = slides.sort(function(a, b) { return a.data.pos - b.data.pos;});
                    res.render(__dirname+'/public/views/visSlideshow.ejs', {slideshowId, slideshowNavn, slideIns, slides: sortSlides});  
