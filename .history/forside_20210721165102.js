...
app.get('/rediger/slideshows/:slideshowId', (req, res) => {
  ...
  db.doc(`slideshows/${slideshowId}`).get().then((docSnapshot) => {
    if (docSnapshot.exists){
      console.log(docSnapshot.data());


      let slides = '';
        db.doc(`slideshows/${slideshowId}/slides`).get().then((docSnapshot) => {
          if (docSnapshot.exists){slides = docSnapshot.data()}
        });

      res.render(__dirname+'/public/views/MasterSlide.ejs', {slideInst: docSnapshot.data(), slides})
    }else{
