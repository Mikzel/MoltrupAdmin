...
const slidesref = firestore.collection(slideshowId);
const slideinsref = firestore.collection(`slideshows`).doc(slideshowId).collection('slides');
...
randomnavn = Math.random().toString(36).substring(2);
  billederef = firebase.storage().ref(`slidebilleder/${randomnavn + filtype}`);
...
firestore.collection('PlanS_1').onSnapshot(function(snapshot) {
        snapshot.docChanges().forEach(function(change) {
  if (change.type=='removed') {
 ...
 <i class="fas fa-trash-alt sletplan"></i>
 <div class="planbilledecon"> <img src="${doc.data().planbillede}"></div>
 <p class="afbruger">${doc.data().bruger}</p>
...
topscroll('.planny');
  ...
  var planbillederef = firebase.storage().ref(`Planlagte billeder/${nytpbnavn + plantype}`);
...
if (tiltim.toString().length<2) {
 ...
 firestore.collection('PlanS_1').doc(nytpbnavn).set({
   fra: parseInt(franum),
   til: parseInt(tilnum),
   planbillede: url,
   postDato: alder,
