...
if (stillinger.includes(posStr)) {
  ...
  .then(json => {
        ...
        <div>
        <p style=" color: ${data.farve || 'white'}; border-bottom: solid 2px ${data.farve}">${json.data.firstName} ${json.data.lastName}</p>
        <p style=" color: ${data.farve || 'white'}; border-bottom: solid 2px ${data.farve}">${beskrivelse}</p>
    </div>  `)
