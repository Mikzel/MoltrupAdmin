...
async function genMiniSkab() {
   ...
   $('.miniSkabParent').append(`<input id="nulstilMiniSkab" onclick="$('.ubrugtCheck').removeAttr('checked')" type="button" value="Nulstil placering">`)
   ...
   if ($('.ubrugtCheck:checked').length > 1) {
        if ($(e.target).prop("checked")) {
            ...
            $('.ubrugtCheck').removeAttr("checked");
            gridVidde(senesteChecked, nyChecked).forEach((x)=>{
                $(`#check_${x}`).attr("checked",true);
                console.log(x, $(`#check_${x}`));
            });
