...
if (data.kategori=='allenyheder') {var kategori = 'alle nyheder';} else {  var kategori = data.kategori;}

if (data.overskrift == false) {
    var kategori = '';
}else if(!data.overskrift){}
else{
    var kategori = data.overskrift;
}
...
<p class="nyhed-kategori" style="font-size: ${data.størrese || '1.4em'}; color: ${data.farve || 'white'}">${kategori}</p>
<ul>
...
var feedurl = 'https://www.dr.dk/nyheder/service/feeds/'+data.kategori;
    ...
    .then(feed => {
      ...
      for(i = 0; i < data.antal; i++){
        $(`#${data.id} *`).css({color: data.farve +'!important'|| 'white'})
          $(`#${data.id} > ul`)
          .append(`
          <li class="nyheder-li-border">
          <p id="autoStr" class="nyhed-overskrift" style="font-size: ${data.overskriftStørrelse || '1.4em'}; color: ${data.farve}">${entry[i].title}</p>
          <p id="autoStr" class="nyhed-beskrivelse" style="font-size: ${data.beskrivelseStørrelse || '1em'}; color: ${data.farve}">${entry[i].description}</p>
      </li>
          `);
...
console.log(imorgen);
    $('main').append(`
    <div class="vagter"   style="padding: 5px; font-size: ${data.størrelse || '2em'}; color: ${data.farve || 'white'} !important; grid-area: ${data.start}; grid-column-end: ${data.sluty}; grid-row-end:${data.slutx};"></div>`)
...
if (element.positionId== 226571 || 
    element.positionId== 231421 || 
    element.positionId== 241530 ||
    element.positionId== 241666 || 
    element.positionId==  928002|| 
    element.positionId== 928003 ||
    element.positionId== 931662 || 

    element.positionId==  226573||
    element.positionId==  231422||
    element.positionId==  928004||
    element.positionId==  931635) {
  ...
  .then(json => {
        ...
        $('.vagter').append(`
        <div>
        <p>${json.data.firstName} ${json.data.lastName}</p>
        <p>${fratid} - ${tiltid}</p>
    </div>  `)
