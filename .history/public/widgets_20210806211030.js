...
function tekstboks(data) {
    var tekst = data.tekst.replaceAll(/\n/g, '<br>');
    console.log(tekst);
    $(`${data.parent||'main'}`).append(`
    <div class="tekstboks widget"  style="display: inline; color: ${data.farve || 'white'};  grid-area: ${data.start};  grid-column-end: ${data.sluty}; grid-row-end: ${data.slutx};">
    <h1 style="color: ${data.farve || 'white'}; text-align: center; font-size: 'larger'">${data.overskrift}</h1>
    <p style="color: ${data.farve || 'white'}; font-size: ${data.størrelse || 'medium'}; text-align:${data.align || 'left'};">${tekst}</p>
</div>
    `);
}
...
function vejr(data){
    switch (data.vistype) {
        case 'vejrpt':
            ...
            $(`${data.parent || 'main'}`).append(`
            <div class=" widget vejrpt vejr " id="${data.id}" style= "grid-area: ${data.start};grid-column-end: ${data.sluty}; grid-row-end: ${data.slutx};">
            <a class="${data.aClass || 'weatherwidget-io'}" href="https://forecast7.com/da/56d208d77/vildbjerg/" data-mode="Current" data-theme="original" data-basecolor="rgba(31, 86, 124, 0)" data-textcolor="${data.farve || '#ffffff'}" data-cloudcolor="${data.farve || '#ffffff'}">Vildbjerg, Denmark</a>
            </div>`);
            break;

        case 'vejrudsigt':
            $(`${data.parent || 'main'}`).append(`
            <div class="widget vejrudsigt vejr " id="${data.id}" style= "grid-area: ${data.start}; grid-column-end: ${data.sluty}; grid-row-end: ${data.slutx};">
            <a class="${data.aClass || 'weatherwidget-io'}" href="https://forecast7.com/da/56d208d77/vildbjerg/" data-label_1="VILDBJERG" data-theme="original" data-basecolor="rgba(31, 86, 124, 0)" data-textcolor="${data.farve || '#ffffff'}" data-cloudcolor="${data.farve || '#ffffff'}" >VILDBJERG</a>
        </div>
            `);
       break;
...
<div class="nedtælling widget" id='${data.id}' style="color: ${data.farve || 'white'} !important; font-size: ${data.størrelse || 'medium'}; grid-area: ${data.start}; grid-column-end: ${data.sluty}; grid-row-end: ${data.slutx}; grid-column-end: ${data.sluty}; grid-row-end:${data.slutx};">
<p style="color: ${data.farve || 'white'} !important; font-size: ${data.størrelse || 'larger'}; font-weight: 'bold';" class='nedtæloverskrift'>${data.tekst || ''}</p>
<p style="color: ${data.farve || 'white'} !important; font-size: ${data.størrelse|| 'medium'};"  class='nedtælud'></p>
</div>`);
...
var feedurl = 'https://www.dr.dk/nyheder/service/feeds/'+data.kategori;
    ...
    .then(feed => {
        ...
        $(data.parent || 'main').append(`
<div class="nyheder widget" id="${data.id}" style="color: ${data.farve || 'white'};  grid-area: ${data.start}; grid-column-end: ${data.sluty}; grid-row-end:${data.slutx};">
<p class="nyhed-kategori" style="font-size: ${data.størrelse|| 'medium'}; color: ${data.farve || 'white'}">${feed.meta.title}</p>
<ul>
...
`);
      ...
      for(i = 0; i < data.antal; i++){
        $(`#${data.id} *`).css({color: data.farve})
          ...
          <p id="autoStr" class="nyhed-overskrift" style="font-size: ${data.størrelse || 'medium'}; color: ${data.farve}">${entry[i].title}</p>
          <p id="autoStr" class="nyhed-beskrivelse" style="font-size: ${data.størrelse || 'medium'}; color: ${data.farve}">${entry[i].description}</p>
      </li>
          `);
...
console.log(imorgen);
        $(`${data.parent || 'main'}`).append(`  
    <div class="dagens-vagter widget"  id="${data.id}"  style="padding: 5px; font-size: ${data.størrelse??'medium'}; color: ${data.farve || 'white'} !important; grid-area: ${data.start}; grid-column-end: ${data.sluty}; grid-row-end:${data.slutx};"></div>`)
...
function slideshow(data) {
    let visInaktiv = '#iframe';
    if (data.visInaktiv) {
       visInaktiv = '';
    }
    $(data.parent || 'main').append(`
        <div class="slideshow widget"  id="${data.id}" data-slideshowParent=${data.slideshowParent} style="grid-area: ${data.start}; grid-column-end: ${data.sluty}; grid-row-end:${data.slutx};">
            <iframe src="${location.origin}/slideshows/${data.slideshowParent.replaceAll(' ','_')}${visInaktiv}" width="100%" height="100%"></iframe>
        </div>
    `)
}
