...
.then((querySnapshot) => {
    querySnapshot.forEach((doc) =>{
        ...
        switch (doc.data().widget) {
            case 'tekstboks':    
            tekstboks({...standard, align: 'center', tekst: doc.data().tekst, overskrift: doc.data().overskrift, farve: doc.data().farve, størrelse: doc.data().størrelse,  id:doc.data().id});
                break;

                case 'vejr':    
                    vejr({...standard, vistype: doc.data().vistype, BG: doc.data().BG, farve: doc.data().farve,  id:doc.data().id});
                break;
                ...
                case 'slideshow': 
                let visInaktiv = {};
                if(location.pathname.includes('/rediger/')){visInaktiv['visInaktiv'] = true}
                slideshow({...standard,...visInaktiv,
