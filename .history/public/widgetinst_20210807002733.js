...
async function genMiniSkab() {
   ...
   $('.ubrugtCheck').change((e)=>{
    if ($(e.target).prop('checked')) {
        var senesteChecked = $(sidsteCheck[sidsteCheck.length-1]);
        var nyChecked = $(e.target);
       ...
       /* if($('.ubrugtCheck:checked').length>1){
            let checkedArr = $('.ubrugtCheck:checked');
          // console.log($('.ubrugtCheck:checked').index($(e.target)));
           checkedArr.splice($('.ubrugtCheck:checked').index($(e.target)), 1)
...
async function gridVidde(gridArea1, gridArea2) {
