...
async function genMiniSkab() {
   ...
   $('.ubrugtCheck').change((e)=>{
    if ($(e.target).prop('checked')) {
        var senesteChecked = $(sidsteCheck[sidsteCheck.length-1]).attr('id').slice($(sidsteCheck[sidsteCheck.length-1]).attr('id').indexOf('_')+1, 0);
        var nyChecked = $(e.target).attr('id').slice($(sidsteCheck[sidsteCheck.length-1]).attr('id').indexOf('_')+1, 0);
 console.log(senesteChecked, nyChecked);
