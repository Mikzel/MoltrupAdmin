...
console.log(imorgen);
    $(`${data.parent||'main'}`).append(`
    <div class="dagens-vagter"   style="padding: 5px; font-size: ${data.størrelse+'px' || '1em'}; color: ${data.farve || 'white'} !important; grid-area: ${data.start}; grid-column-end: ${data.sluty}; grid-row-end:${data.slutx};"></div>`)
...
if (stillinger.includes(posStr)) {
  ...
  .then(json => {
        ...
        <p style=" color: ${data.farve || 'white'}">${beskrivelse}</p>
            ...
            $('.dagens-vagter div').css('border-bottom',`solid 2px ${data.farve||'white'}`);
