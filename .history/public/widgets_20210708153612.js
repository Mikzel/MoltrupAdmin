    ...
    .then(feed => {
      ...
      for(i = 0; i < data.antal; i++){
        $(`#${data.id}`).css({color: data.farve +'!important'|| 'white'})
          ...
          <p id="autoStr" class="nyhed-overskrift" style="font-size: ${data.overskriftStørrelse || '1.4em'}";>${entry[i].title}</p>
          <p id="autoStr" class="nyhed-beskrivelse" style="font-size: ${data.beskrivelseStørrelse || '1em'}";>${entry[i].description}</p>
      </li>
          `);
