...
function gridSammenhæng(liste) {
    ...
    if (Array.isArray(liste)) {
        ...
        $(liste).each((i, el)=>{
          ...
          if(!kontrol[kord[0]]) { kontrol[kord[0]]=[] };
          kontrol[kord[0]].push(parseInt(kord[1]));
        })

        var sidst;
        for (const property in kontrol) {
         kontrol[property].forEach((x)=>{
             if(sidst){
                 if(x==sidst+1){
                    sidst = x;
                    console.log(kontrol[property]);
                 }else{
                    return false;
                 }
             }
