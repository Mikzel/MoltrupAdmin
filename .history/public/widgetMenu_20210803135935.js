      ...
      if(forbudtNavn){
        ...
        await db.collection('slideshows').doc(navn).get().then((docSnapshot)=>{
          if(docSnapshot.exists){
            ...
            db.collection('slideshows').doc(navn).set({toggleaktiv: true, toggleovergang: true, varighed: 10})
            db.collection('slideshows').doc(navn).collection('slides').set();
            db.collection('slideshows').doc(navn).collection('planlagteSlides').set();
          }
        });
