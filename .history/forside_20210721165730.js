...
app.get('/rediger/slideshows/:slideshowId', (req, res) => {
  ...
  db.collection(`slideshows`).doc(slideshowId).listCollections().then((subCollections) => {
    if (subCollections.exists){
      console.log(subCollections.data());


    
      res.render(__dirname+'/public/views/MasterSlide.ejs', {slideshow: subCollections.data()})
    }else{
